		...
		if(num_of_reqs == 32){
			...
			while(part<total_round){
				...
				for(int i=0; i<GROUP; i++){
					if(part*GROUP+i < 32) thread_handle_mulreq();
				}
				for(int i=0; i<GROUP; i++){
					if(part*GROUP+i < 32)  pthread_join(tid[part*GROUP+i][0], NULL);
