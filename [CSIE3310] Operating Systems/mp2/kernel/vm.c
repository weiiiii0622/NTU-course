...
// va =  (base + (idx << 9*level) << PGSHIFT

#define TAB() printf("    ")
#define CONT() printf("|   ")
#define PREFIX() printf("+-- ")

//int isLast[3]; // 0:no 1:yes

void printer(pagetable_t pagetable, int level, uint64 base, int isLast[]){
  if(level < 0) return;
  ...
  for(int idx=0; idx<512; idx++){
    ...
    if(*pte & PTE_V || *pte & PTE_S){
      ...
      va = ((uint64)(base + (idx << 9*level))) << PGSHIFT;
      ...
      pa = PTE2PA(*pte);
      if(*pte & PTE_V){
        printf("%d: pte=%p va=%p pa=%p", idx, pte, va, pa);
      }
      else{
        blockno = PTE2BLOCKNO(*pte);
        printf("%d: pte=%p va=%p blockno=%p", idx, pte, va, blockno);
      }
      ...
      printf("\n");
      if(*pte & PTE_V){
        printer((pagetable_t)pa, level-1, (uint64)(base +(idx << 9*level)), isLast);
      }
    }
  }
}
...
int madvise(uint64 base, uint64 len, int advice) {
  ...
  // handle memory region bound
  // [base, base+len) <-> [0, p->sz)
  if(base+len > p->sz) return -1;

  // get va
  uint64 begin = PGROUNDDOWN(base), end = PGROUNDDOWN(base+len-1);
  if(advice == MADV_NORMAL){
    ...
    for(uint64 va=begin; va<=end; va+=PGSIZE){
      ...
      if(*pte & PTE_V){
        ...
        *pte |= PTE_S;
        *pte &= ~PTE_V;
        // write into disk & free physical pg
        begin_op();
        pg = PTE2PA(*pte);
        blk = balloc_page(ROOTDEV);
        write_page_to_disk(ROOTDEV, (char*)pg, blk);
        kfree((void*)pg);
        end_op();

        // redirect to disk block
