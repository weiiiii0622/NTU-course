...
bool DeckCmp (const card &x, const card &y) {
  return x.id < y.id;
}

int my_find_stack_number (int id) {
  if (id < my_n[0]) {
    return 0;
  }
  if (id < my_n[0] + my_n[1]) {
...
void init (vector<card> stack_1, vector<card> stack_2, vector<card> stack_3) {
	...
	sort(deck_main.begin(), deck_main.end(), DeckCmp);
...
bool Myfail (Board b, struct move m, int player) {
  ...
  switch (m.type) {
    ...
    case 2:
      ...
      if ((!b.buffer[my_find_stack_number(m.card_id)].count(m.card_id)) && (!b.imprisoned[player].count(m.card_id))) {
        return true;
      }
    ...
    case 3:
      ...
      if (!b.buffer[my_find_stack_number(m.card_id)].count(m.card_id)) {
        return true;
      }
...
bool Myplay (Board &b, struct move m, int player) {
  switch (m.type) {
    ...
    case 2:
      ...
      if (b.buffer[my_find_stack_number(m.card_id)].count(m.card_id)) {
        b.buffer[my_find_stack_number(m.card_id)].erase(m.card_id);
      }
      else {
...
void my_check_buffer (Board &b) {
  for (int i = 0; i < 3; i++) {
    while (b.buffer[i].size() < 4 && b.cnt[i] < my_n[i]) {
